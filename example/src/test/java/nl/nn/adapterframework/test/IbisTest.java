...
public class IbisTest {
	...
	public void test() {
		...
		System.setProperty("ibistest.active", "true");
		debug("***start***");
		...
		debug("adapters started [" + adaptersStarted + "] from ["
				+ adaptersStarted + "]");
		assertEquals(adaptersCount, adaptersStarted);

		//boolean larvaFull = appConstants.getBoolean("larva.full", false);
		boolean larvaFull = true;
		...
		if (result == null) {
			...
			if (scenarios == null || scenarios.size() == 0) {
				...
				debug("scenarios passed [" + scenariosPassed + "] from ["
						+ scenariosCount + "]");
				assertEquals(scenariosCount, scenariosPassed);
			}
		}
